...
Integer indicating the lowest temperature the heat setpoint can be configured as. Default is 4. Best to use `_f `and \_\``_c and not the older Celsius x 10 capability. This capability can be changed through a`DYNAMIC\_CAPABILITIES\_CHANGED\` notification.
